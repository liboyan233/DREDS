...
class SwinDRNetTrainer():
    ...
    def prepare_data(self):
        ...
        print('valsize', val_size, 'percent', self.percentageDataForVal)
    ...
    def inference(self):
        ...
        print('num', len(self.val_loader))
        for i, batch in enumerate(tqdm(self.val_loader)):
            print(batch.keys())
            # import matplotlib.pyplot as plt
            # depth_map = batch['syn_depth'][0, 0].cpu().numpy()
            # plt.imshow(depth_map, cmap='viridis')  # Use viridis colormap for better perception of depth
            # plt.colorbar(label='Depth')  # Add color bar to indicate depth values
            # plt.title('Depth Map')  # Set title
            ...
            with torch.no_grad():
                ...
                i = 0
                fig, axs = plt.subplots(2, 2, figsize=(10, 8))  # Create subplots for each image

                depth_map_optimized = outputs_depth[i, 0].cpu().numpy()
                dp = batch['sim_depth']
                print('dp shape', dp.shape, torch.max(dp))
                depth_map_input = batch['sim_depth'][i,0].cpu().numpy()
                # depth_map_input[depth_map_input>3] = 0  # filter the raw depth map
                color_img = batch['rgb'][i].permute(1,2,0)
                color_img = color_img.cpu().numpy()

                print('max_depth', np.max(depth_map_optimized), ';', np.max(depth_map_input))
                #       ';', np.max(depth_map_gt), ';')

                img = axs[0,0].imshow(depth_map_optimized, cmap='viridis')
                fig.colorbar(img, ax=axs[0,0])
                axs[0,0].set_title('predict depth map')
                axs[0,0].axis('off') 

                img = axs[0,1].imshow(depth_map_input, cmap='viridis')  # Use viridis colormap for better perception of depth
                fig.colorbar(img, ax=axs[0,1])
                axs[0,1].set_title('raw depth map')
                axs[0,1].axis('off')

                # img = axs[1,0].imshow(depth_map_gt, cmap='viridis')
                # fig.colorbar(img, ax=axs[1,0])
                # axs[1,0].set_title('gt depth map')
                # axs[1,0].axis('off')
