...
class SwinDRNetTrainer():
    def __init__(self, args, model, device_list, continue_ckpt_path):
        ...
        print('trainer length', len(self.train_loader))
        if args.n_gpu > 1:
    ...
    def inference(self):
        ...
        for i, batch in enumerate(tqdm(self.val_loader)):
            ...
            with torch.no_grad():
                # print(batch.keys()) 
                # ['rgb', 'sim_xyz', 'sim_depth', 'output_depth', 'syn_depth', 'nocs_map', 'sem_mask', 'ins_mask', 'ins_w/o_others_mask', 'scale']
                # print(batch[''])
                # outputs_depth, normal_labels, normal_pred, confidence_sim_ds, confidence_initial, \
